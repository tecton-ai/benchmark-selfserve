...
from tecton import AutoscalingConfig, Aggregate

auto = FeatureServerGroup(
    name='autoscaling-benchmark',
    scaling_config=AutoscalingConfig(min_nodes=30, max_nodes=50),
    owner="yan@tecton.ai",
    description="Server group benchmark tests",
    options={
        "REQUEST_TIMEOUT":"30s"
    },
)
...
customer = Entity(name='customer', join_keys=[Field("cust_id", String)])
merchant = Entity(name='merchant', join_keys=[Field("merchant_id", String)])

from tecton import Aggregate, Attribute
...
@batch_feature_view(
    ...
    features=[
        Aggregate(input_column=Field(f'test_{i}', Float64), function='sum', time_window=timedelta(days=28))
        for i in range(62)
    ],
    online=True,
    offline=False,
...
@batch_feature_view(
    ...
    features=[
        Aggregate(input_column=Field(f'test_{i}', Float64), function='sum', time_window=timedelta(days=7))
        for i in range(62)
    ],
    online=True,
    offline=False,
...
@batch_feature_view(
    ...
    aggregation_interval=timedelta(days=1),
    features=[
        Aggregate(input_column=Field(f'test_{i}', Float64), function='sum', time_window=timedelta(days=1))
        for i in range(50)
    ],
    online=True,
    offline=False,
